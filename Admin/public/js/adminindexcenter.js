...
form[0].addEventListener('submit', function (e) {
    e.preventDefault();
    let lo = loc();
    let img = document.querySelector('#xh').files[0].name;
    let title = document.querySelector('#username').value;
    let brief = document.querySelector('#con').value;
    let jj = document.querySelector('#jj').value;
    let http = document.querySelector('#http').value;
    lo.push({
        ...
        success: function (res) {
            if (res == true) {
                ...
                tbody.innerHTML = res.map(function (item) {
                    return `
                        <tr>
                            <td><img src="../../public/images/${item.img}"></td>
                            <td>${item.title}</td>
                            <td>${item.brief}</td>
                            <td>${item.jj}</td>
                            <td>${item.http}</td>
...
function update(userid) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            document.querySelector('#username1').value = res.title;
            document.querySelector('#con1').value = res.brief;
            document.querySelector('#jj1').value = res.jj;
            document.querySelector('#http1').value = res.http;
            document.querySelector('.pp1').innerHTML = `<img src="../../public/images/${res.img}">`;
        }
    })
}
...
btn.onclick = function () {
    let lc = loc();
    let img = document.querySelector('#xh1').files[0];
    if (img == undefined) {
        img = lc[0].img;
    } else {
        img = img.name;
    ...
    let title = document.querySelector('#username1').value;
    let brief = document.querySelector('#con1').value;
    let jj = document.querySelector('#jj1').value;
    let http = document.querySelector('#http1').value;
    let obj = {
        id: uid,
        img,
        title,
