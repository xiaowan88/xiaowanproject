...
// 获取本地存储数据
function loc() {
    return JSON.parse(localStorage.getItem('messagepic')) || [];
}

// 渲染页面
Mock.mock("/api/xr", "get", function () {
    let goodsarr = JSON.parse(localStorage.getItem('messagepic')) || [];
    if (goodsarr == null || goodsarr == [] || goodsarr == undefined || goodsarr == '[]') {
        return false;
    }
    return goodsarr;
})
...
// 数据的添加
Mock.mock("/mock/add", "post", function (obj) {
    let newObj = JSON.parse(obj.body);
    let loca = loc();
    loca.push({
        id: loca.length ? loca[loca.length - 1].id + 1 : 1,
        pic: newObj.pic
    });
    localStorage.setItem('messagepic', JSON.stringify(loca));
})
...
Mock.mock("/api/upd", "post", function (obj) {
    ...
    bd[index] = garr[0];
    localStorage.setItem('messagepic', JSON.stringify(bd));
    return true;
})
