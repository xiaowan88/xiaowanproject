function messg() {
    return JSON.parse(localStorage.getItem('goodsnews')) || [];
}
...
Mock.mock("/mock/lbt", "post", function (res) {
    ...
    localStorage.setItem('goodsnews', JSON.stringify(mess));
})
...
Mock.mock("/mock/del", "post", function (res) {
    let message = JSON.parse(localStorage.getItem('goodsnews'));
    for (let i = 0; i < message.length; i++) {
        if (message[i].id == res.body) {
            if (confirm('您确定要删除这条数据吗？')) {
                message.splice(i, 1);
                localStorage.setItem('goodsnews', JSON.stringify(message))
            }
        }
    }
})
