...
Mock.mock("/mock/lbt", "post", function (res) {
    ...
    mess.push({
        ...
        title: res1.title,
        classid: mess.length ? mess[mess.length - 1].classid + 1 : 1
    })
    localStorage.setItem('allgoodsclass', JSON.stringify(mess));
})
...
Mock.mock("/mock/del", "post", function (res) {
    let message = JSON.parse(localStorage.getItem('allgoodsclass'));
    for (let i = 0; i < message.length; i++) {
        if (message[i].id == res.body) {
            if (confirm('您确定要删除这条数据吗？')) {
                message.splice(i, 1);
                localStorage.setItem('allgoodsclass', JSON.stringify(message))
            }
        }
    }
})
